...
function decodeAgentFromUrl(search: string): Partial<Agent> | null {
  try {
    const params = new URLSearchParams(search);
    const d = params.get("d");
    if (!d) return null;
    return JSON.parse(decodeURIComponent(escape(atob(d)))) as Partial<Agent>;
  } catch {
    return null;
  }
}

function toFullAgent(p: Partial<Agent>): Agent {
  return {
    id: p.id ?? "",
    nome: p.nome ?? "",
    ...
    validade: p.validade ?? "",
    foto: p.foto ?? "",
    equipamentoTipo: p.equipamentoTipo ?? "",
    equipamentoMarca: p.equipamentoMarca ?? "",
    equipamentoNrSerie: p.equipamentoNrSerie ?? "",
  };
}
...
export default function AgentPublicView() {
  ...
  const decoded = !stored ? decodeAgentFromUrl(search) : null;
  const partial: Partial<Agent> | null = stored ?? decoded;
  ...
  const agent = toFullAgent(partial);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <p style={{
            color: "#374151", fontWeight: 500, fontSize: 11, margin: "2px 0 0",
          }}>
        ...
        {/* Colored stripe */}
        <div style={{
          display: "flex", width: 120, height: 5,
      ...
      <div style={{
        ...
        gap: 12, width: "100%", maxWidth: W + 32,
        padding: "24px 16px 0",
      }}>
